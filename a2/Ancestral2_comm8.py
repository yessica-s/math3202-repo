...
for i in I:
    ...
    for d in T:
        #max hours worked per technician - daily
        m.addConstr(gp.quicksum(X[i,j]*Tasks[j]['duration'] for j in J if Tasks[j]['day']==d) <= maxHoursDaily) #(comm8)
...
for d in T:
    for i in I:
        m.addConstr(10000*Y[i,d] >= gp.quicksum(X[i,j] for j in J if Tasks[j]['day']==d)) #(comm8)
